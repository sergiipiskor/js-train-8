...
function createArray(...elements) {
  ...
  } else return "Помилка. Не передано жодного елементу";
...
function findElementIndexes(arr, element) {
  ...
  if (!Array.isArray(arr)) {
    ...
    return Array(arr.indexOf(element), arr.lastIndexOf(element));
  }
...
function copyAndSwapElements(arr, target, start, end) {
  ...
  else return arr.copyWithin(target, start, end);
}
...
function customUnshift(arr, ...elements) {
  ...
  if (Array.isArray(arr)) {
    ...
    for (let newLength = initialLength - 1; newLength >= 0; i--) {
      newLength = arr.unshift(...elements);
...
function customAt(arr, index) {
  ...
  if (!Array.isArray(arr)) {
    ...
    return "undefined";
  } else if (typeof arr.at(index) == "string") {
  ...
  return arr.at(index); // Отримуємо елемент масиву за заданим індексом
...
function customIncludes(arr, element) {
  ...
  let elemincl = arr.includes(element);
  if (elemincl) {
    let result = arr.filter((word) => word === element);
